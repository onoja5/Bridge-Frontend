...
const MentorshipBackup: React.FC = () => {
  ...
  const fetchAndSetMentors = async () => {
    try {
      const { mentors: fetchedMentors, totalPages } = await fetchMentors(
        page,
        limit,
      );

      console.log('fetchedMentors', fetchedMentors);
      ...
      <section>
        ...
        {Object.keys(groupedMentors).length > 0 ? (
          ...
          <p className='text-sm text-gray-500'>
            No mentors grouped by specialty.
          </p>
        )}
      </section>
    </div>
...
export default MentorshipBackup;
