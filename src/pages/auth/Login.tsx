...
export default function Login() {
  ...
  useEffect(() => {
    if (isAuthenticated && userData?.role) {
      const redirectPath = userData.role === 'MENTOR'
        ? '/mentor/dashboard'
        : userData.role === 'PARTNER'
        ? '/partner/dashboard'
        : '/talent/dashboard';
      window.location.href = redirectPath; // Use window.location.href for immediate redirect
    }
  }, [isAuthenticated, userData]);

  // Redirect to select-user-type if role is not set and user is authenticated
  if (isAuthenticated && !userData?.role) {
    console.warn('User role missing after login:', userData);
